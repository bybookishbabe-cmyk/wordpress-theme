(function () {
	...
	function downloadPanel() {
		...
		function ensurePanel() {
			...
			panel.className = 'bbb-midnight-downloadPanel';
			panel.setAttribute('role', 'status');
			panel.setAttribute('aria-live', 'polite');
			panel.innerHTML = [
				'<div class="bbb-midnight-downloadPanel__copy">',
				'<strong>download started</strong>',
				'<span>you can keep browsing the theme.</span>',
				'</div>',
				'<button type="button" class="bbb-midnight-downloadPanel__close" aria-label="close download message">×</button>'
			].join('');
		...
		root.addEventListener('click', function (event) {
			...
			if (!link || !isPwaDisplay() || !link.href) {
				return;
			}
	...
	function sectionNav() {
		root.addEventListener('click', function (event) {
			var link = event.target && event.target.closest ? event.target.closest('.bbb-midnight-kit__nav a[href^="#"]') : null;
			if (!link) {
				return;
			}
			...
			var nav = root.querySelector('.bbb-midnight-kit__nav');
			var adminBar = document.getElementById('wpadminbar');
			var offset = (nav ? nav.getBoundingClientRect().height : 0) + (adminBar ? adminBar.getBoundingClientRect().height : 0) + 18;
	...
	var canRunDecorativeMotion = window.matchMedia('(max-width: 899px)').matches;
	...
	function starfield() {
		...
		window.setTimeout(shoot, 900);
		window.setTimeout(shoot, 2400);
		window.setInterval(shoot, 3100);
		draw();
	}
	...
	function swipeRails() {
		var rails = Array.from(root.querySelectorAll('.bbb-midnight-kit__nav, .bbb-midnight-inserts, .bbb-midnight-downloads__grid, .bbb-midnight-books, .bbb-midnight-playlists, .bbb-midnight-quotes'));
		...
		rails.forEach(function (rail) {
			...
			function endDrag(event) {
				...
				if (event && rail.hasPointerCapture && rail.hasPointerCapture(event.pointerId)) {
			...
			rail.addEventListener('pointerdown', function (event) {
				...
				if (rail.classList.contains('bbb-midnight-kit__nav') && event.target && event.target.closest && event.target.closest('a')) {
					return;
				}
