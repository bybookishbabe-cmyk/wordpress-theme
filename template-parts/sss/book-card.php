...
$ku_url        = function_exists('bbb_normalize_url_value') ? bbb_normalize_url_value(bbb_sss_card_field('kindle_unlimited_link', $book_id, bbb_sss_card_field('ku_link', $book_id, ''))) : (string) bbb_sss_card_field('kindle_unlimited_link', $book_id, bbb_sss_card_field('ku_link', $book_id, ''));
...
if ($series_value instanceof WP_Post) {
	$series_handle = $series_value->post_name;
} elseif (is_numeric($series_value)) {
	$series_handle = (string) get_post_field('post_name', (int) $series_value);
} else {
	$series_handle = sanitize_title((string) $series_value);
}

$series_name = (string) bbb_sss_card_field('series_name', $book_id, '');
if ('' === $series_name && $series_value instanceof WP_Post) {
	$series_name = get_the_title($series_value);
} elseif ('' === $series_name && is_numeric($series_value)) {
	$series_name = get_the_title((int) $series_value);
}
...
$trope_urls    = array_map(
	static function (array $trope): string {
		...
		return '/' . ('-books' === substr($slug, -6) ? $slug : $slug . '-books');
	},
	$tropes
);
...
<button
	...
	data-ku-url="<?php echo esc_attr($ku_url); ?>"
