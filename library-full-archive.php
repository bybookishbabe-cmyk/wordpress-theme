...
$archive_books = array_values(
	array_filter(
		$books,
		static function (WP_Post $book) use ($current_month): bool {
...
$initial_count = 24;
$initial_books = array_slice($archive_books, 0, $initial_count);
?>
<section class="sss-lib__archiveSection" data-archive-section>
	...
	<div
		class="sss-lib__grid"
		id="sssArchiveGrid"
		data-lazy-archive="true"
		data-archive-endpoint="<?php echo esc_url(rest_url('bbb/v1/library/archive')); ?>"
		data-archive-offset="<?php echo esc_attr((string) count($initial_books)); ?>"
		data-archive-total="<?php echo esc_attr((string) count($archive_books)); ?>"
		data-archive-limit="24"
	>
		<?php foreach ($initial_books as $book) : ?>
