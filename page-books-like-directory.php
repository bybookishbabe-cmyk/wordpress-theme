...
$blog_guides = function_exists('bbb_books_like_blog_guide_posts') ? bbb_books_like_blog_guide_posts() : array();
?>
...
<main id="MainContent" class="content-for-layout focus-none" role="main" tabindex="-1">
	<section class="bbb-books-like-directory">
		<div class="bbb-books-like-directory__wrap">
			...
			<?php if ($groups || $blog_guides) : ?>
				<div class="bbb-books-like-directory__groups">
					<?php if ($groups) : ?>
						<section class="bbb-books-like-directory__group" aria-label="books-like page templates">
							<h2 class="bbb-books-like-directory__groupTitle">if you liked pages</h2>
					...
					<?php if ($blog_guides) : ?>
						<section class="bbb-books-like-directory__group" aria-label="books-like blog guides">
							<h2 class="bbb-books-like-directory__groupTitle">blog guides</h2>
							<div class="bbb-books-like-directory__grid">
								<?php foreach ($blog_guides as $item) :
									$post   = $item['post'];
									$source = $item['source'];
									$book   = $source instanceof WP_Post ? bbb_books_like_book_data($source->ID) : array();
									$cover  = (string) ($book['cover'] ?? get_the_post_thumbnail_url($post->ID, 'large'));
									$subtitle = wp_strip_all_tags(get_the_excerpt($post));
									if ('' === $subtitle && !empty($book['mini'])) {
										$subtitle = (string) $book['mini'];
									}
								?>
									<a class="bbb-books-like-directory__card" href="<?php echo esc_url(get_permalink($post)); ?>">
										<div class="bbb-books-like-directory__cardMedia">
